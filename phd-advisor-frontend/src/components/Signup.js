...
const Signup = ({ onNavigateToLogin, onNavigateToHome }) => {
  ...
  const knowledgeLevels = config?.login?.knowledge_levels?.length
    ...
    : config?.login?.academic_stages?.length
      ...
      : [
          { value: '', label: 'Where are you on healthy eating?' },
  ...
  const validateForm = () => {
    ...
    if (!formData.academicStage) {
      newErrors.academicStage = 'Please select where you are on healthy eating';
    }
      ...
      <div className="signup-container">
        ...
        <div className="signup-form-container">
          <form onSubmit={handleSubmit} className="signup-form">
            ...
            {/* Food focus */}
            <div className="form-group">
              <span className="form-label">What food goal are you aiming for?</span>
              <div className="career-focus-options">
                {[
                  { value: 'Clean eating basics', label: 'Get started' },
                  { value: 'More fruits & vegetables', label: 'More produce' },
                ].map((opt) => (
                  <label
                    key={opt.value}
