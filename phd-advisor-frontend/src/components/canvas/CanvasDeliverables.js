...
export const TEMPLATES = [
  {
    ...
    desc: 'Goals → Produce focus → Prep rhythm → Weekly cadence → Shopping → Tracking',
    icon: 'book',
    mode: 'paper',
    sections: [
      { id: 'abstract', name: 'Overview', target: 250, hint: 'One paragraph: current eating habits, produce goal, and a success metric.', checks: ['hasNumber', 'hasFinding'] },
      { id: 'intro', name: 'Goals & constraints', target: 1000, hint: 'Start clean vs produce focus, kitchen time, and hours/week available.', checks: ['hasNumber'] },
      { id: 'methods', name: 'Meals & produce focus', target: 800, hint: 'Primary/backup meal lanes and a short produce shortlist.', checks: [] },
      { id: 'results', name: 'Weekly cadence', target: 800, hint: 'Prep blocks, produce targets, and market-day handling.', checks: ['hasNumber'] },
      { id: 'discussion', name: 'Kitchen checklist', target: 1000, hint: 'Pantry staples, washed produce, snack swaps, and a simple shopping list.', checks: [] },
      { id: 'refs', name: 'Tracking system', target: 0, hint: 'Board columns, metrics to review every Friday.', checks: [] },
    ],
  },
  {
    id: 'thesis-chapter',
    name: 'Weekly Eating Review',
    desc: 'Overview → What you cooked → What slipped → What to change',
    icon: 'book',
    mode: 'paper',
    sections: [
      { id: 'overview', name: 'Overview', target: 200, hint: 'What week this covers and the goal.', checks: [] },
      { id: 'background', name: 'Meals cooked', target: 1500, hint: 'Produce used, recipes tried, and leftover wins.', checks: ['hasNumber'] },
      { id: 'methods', name: 'Prep & shopping', target: 1500, hint: 'Market notes, prep blocks, and snack swaps.', checks: ['hasNumber'] },
      { id: 'results', name: 'Results', target: 2000, hint: 'What tasted good, what you skipped, and energy notes.', checks: ['hasNumber'] },
  ...
  },
  {
    ...
    mode: 'document',
    sections: [
      { id: 'personal', name: 'Food background', target: 1500, hint: 'Current habits, kitchen setup, and motivation. Write as a story.', checks: [] },
      { id: 'research', name: 'Goal plan', target: 1000, hint: 'Specific target (produce, snacks, or weeknight dinners) and how you will get there.', checks: ['hasNumber'] },
    ],
  },
  {
    id: 'conference-abstract',
    name: 'Weekly Check-in',
    desc: 'Single section, ~250 words. Lead with produce and prep numbers.',
    icon: 'send',
    mode: 'document',
    sections: [
      { id: 'abs', name: 'Check-in', target: 250, hint: 'One paragraph: meals cooked, produce eaten, blockers, next focus.', checks: ['hasFinding', 'hasNumber'] },
    ],
  },
  {
    id: 'defense-slides',
    name: 'Eating Plan Presentation',
    desc: 'Title → Goals → Produce → Cadence → Kitchen setup → Risks → Q&A',
    icon: 'kanban',
    mode: 'slides',
    sections: [
      { id: 'title', name: 'Title slide', target: 30, hint: 'Plan name, your name, date.', checks: [] },
      { id: 'outline', name: 'Outline', target: 60, hint: '5–7 bullets covering the plan.', checks: [] },
      { id: 'background', name: 'Background', target: 200, hint: 'Current habits and why this food focus.', checks: [] },
      { id: 'question', name: 'Goal', target: 80, hint: 'Single measurable goal for the next two weeks.', checks: [] },
      { id: 'methods', name: 'Approach', target: 200, hint: 'Shopping, cadence, and prep plan.', checks: ['hasNumber'] },
      { id: 'results', name: 'Expected results', target: 300, hint: 'Realistic produce and homemade-meal targets by week.', checks: ['hasNumber'] },
      { id: 'discussion', name: 'Risks', target: 200, hint: 'Time conflicts, empty fridge, takeout defaults.', checks: ['hasLimit'] },
  ...
  },
  {
    ...
    mode: 'poster',
    sections: [
      { id: 'title', name: 'Title & Eater', target: 30, hint: 'Food focus, your name, week.', checks: [] },
      { id: 'intro', name: 'Goal', target: 200, hint: 'Where you started and where you are headed.', checks: [] },
      { id: 'methods', name: 'Approach', target: 200, hint: 'Shopping, cadence, kitchen setup.', checks: ['hasNumber'] },
      { id: 'results', name: 'Results', target: 250, hint: 'Headline produce and homemade-meal numbers.', checks: ['hasNumber'] },
      { id: 'discussion', name: 'Next steps', target: 200, hint: 'What it means + what is next.', checks: ['hasLimit'] },
      { id: 'refs', name: 'Notes / Credits', target: 80, hint: 'Advisors, cookbooks, links.', checks: [] },
    ],
  },
  {
    id: 'cv',
    name: 'Weekly Meal Notes',
    desc: 'Standard sections: Kitchen setup · Meals · Prep · Habits · Skills.',
    icon: 'user',
    mode: 'document',
    sections: [
      { id: 'header', name: 'Header', target: 40, hint: 'Name, week of, kitchen setup.', checks: [] },
      { id: 'education', name: 'Kitchen setup', target: 100, hint: 'Tools, pantry staples, and time available.', checks: [] },
      { id: 'publications', name: 'Meals cooked', target: 300, hint: 'Homemade meals with produce + one simple win each.', checks: ['hasNumber'] },
      { id: 'talks', name: 'Recipes to try', target: 150, hint: 'New dishes with ingredients and outcomes.', checks: [] },
      { id: 'awards', name: 'Habit wins', target: 100, hint: 'Snack swaps, leftover reuse, market finds.', checks: [] },
      { id: 'service', name: 'Shopping notes', target: 100, hint: 'Produce that matched this week\'s meals.', checks: [] },
      { id: 'skills', name: 'Skills', target: 60, hint: 'Prep methods and flavor techniques you used.', checks: [] },
    ],
  },
  {
    id: 'cover-letter',
    name: 'Kitchen Note / Share',
    desc: 'For sharing a recipe, market find, or prep plan.',
    icon: 'send',
    mode: 'document',
    sections: [
      { id: 'header', name: 'Header', target: 40, hint: 'Date, recipient, greeting.', checks: [] },
      { id: 'opener', name: 'Opening paragraph', target: 100, hint: 'Recipe or market find + why you are sharing it.', checks: [] },
      { id: 'body', name: 'How to make it', target: 250, hint: '2–3 concrete steps and produce notes. Numbers > adjectives.', checks: ['hasNumber'] },
      { id: 'fit', name: 'Why this meal', target: 150, hint: 'Specific taste, time, or leftover reason.', checks: [] },
      { id: 'close', name: 'Close', target: 60, hint: 'Thanks + clear next step + signature.', checks: [] },
    ],
  },
  {
    id: 'linkedin-about',
    name: 'About My Eating Goals',
    desc: 'Headline, story, and habit blurbs you can scan in 30 seconds.',
    icon: 'user',
    mode: 'document',
    sections: [
      { id: 'framing', name: 'Framing choices', target: 80, hint: 'Rewrite vs polish · clean eating vs more produce · tone (warm / crisp / practical).', checks: [] },
      { id: 'about-me', name: 'About Me narrative', target: 200, hint: 'Who you are, what you want to eat, and proof points — plain language you can skim.', checks: [] },
      { id: 'headline', name: 'Goal headline', target: 40, hint: 'Food lane + proof + target (≤220 chars). No buzzword soup.', checks: [] },
      { id: 'about', name: 'About this season', target: 300, hint: '3–5 short paragraphs or bullets tied to your eating goals.', checks: ['hasNumber'] },
      { id: 'experience', name: 'Habit blurbs', target: 250, hint: 'Top 1–2 meals or prep wins with action + outcome lines.', checks: ['hasNumber'] },
      { id: 'alignment', name: 'Plan ↔ kitchen check', target: 100, hint: 'Same story, meals, and tools — no invented metrics.', checks: [] },
    ],
  },
  {
    id: 'irb-protocol',
    name: 'Meal Prep Sheet',
    desc: 'Market research, likely recipes, and step-by-step answers.',
    icon: 'shield',
    mode: 'document',
    sections: [
      { id: 'overview', name: 'Prep overview', target: 200, hint: 'Market or kitchen, meal focus, format (batch/weeknight), date.', checks: [] },
      { id: 'background', name: 'Recipe research', target: 400, hint: 'Produce, recent finds, flavors, and why you care.', checks: [] },
      { id: 'aims', name: 'Must-haves', target: 250, hint: 'Ingredients and constraints from the recipe.', checks: [] },
      { id: 'participants', name: 'Your proof points', target: 300, hint: 'Match each must-have to a pantry item or swap.', checks: ['hasNumber'] },
      { id: 'procedures', name: 'Step-by-step', target: 500, hint: 'Write 3–5 full steps you will actually cook.', checks: ['hasNumber'] },
      { id: 'risks', name: 'Hard spots', target: 200, hint: 'Time, missing produce, “what if dinner falls apart.”', checks: ['hasLimit'] },
      { id: 'benefits', name: 'Questions for advisors', target: 100, hint: '3 smart questions that show you read the recipe.', checks: [] },
      { id: 'consent', name: 'Logistics', target: 200, hint: 'Tools, thaw plan, leftover containers, thank-you note to yourself.', checks: [] },
      { id: 'data', name: 'After-cooking notes', target: 200, hint: 'What went well, what to improve, next cook date.', checks: [] },
    ],
  },
  {
    id: 'meeting-prep',
    ...
    sections: [
      ...
      { id: 'progress', name: 'Progress since last meeting', target: 200, hint: 'Meals, produce, snack swaps — with numbers.', checks: ['hasNumber'] },
      ...
      { id: 'questions', name: 'Questions', target: 150, hint: 'Open questions for the food advisor.', checks: [] },
        ...
        <div className="page-header">
          <div>
            ...
            <div className="page-sub">
              Open a draft first, then use Export (Markdown, HTML, or Print / Save as PDF). Drafts auto-save.
