"""Seed Heidi's bundled knowledge/*.md files into a shared Chroma session.

User uploads still work per-chat. This pack makes the shipped corpus searchable
without requiring every guest to re-upload markdown files.
"""
...
KNOWLEDGE_PACK_SESSION_ID = "__hea_knowledge_pack__"
...
def seed_bundled_knowledge_pack() -> dict:
    ...
    for path in iter_knowledge_markdown(knowledge_dir):
        # Namespace by advisor folder so coconut reviews don't collide across personas
